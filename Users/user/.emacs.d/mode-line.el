...
(defun aligned-mode-line (left-pad left center right right-pad)
  (let* ((left-pad-f  `(format-mode-line ,left-pad))
         ...
         (available `(if (cl-oddp ,available)
                         (+ 1 ,available)
                         ,available))
         (left-cutoff `(+ 4 (length ,left-pad-f)))
         (right-cutoff `(+ 4 (lenght ,right-pad-f)))
         (left      `(if (> (length ,left-f) (+ (length ,left-pad-f) ,available))
                         (concat (seq-take ,left-f (- ,available ,left-cutoff)) "... ")
                         ,left-f))
         (right     `(if (> (length ,right-f) (+ (length ,right-pad-f) ,available))
                         (concat " ..." (seq-drop ,right-f (+ (- ,right-f ,available) ,right-cutoff)))
                         ,right))
